...
class _tutor_mainState extends State<tutor_main> {
  ...
  Future<bool> GetServices() async {
    ...
    final String apiUrl =
        "http://localhost:3000/Edume/v1/tutor/services/" + shPr.getString("id");
    ...
    if (response.statusCode == 200) {
      for (int i = 0; i < (jsonDecode(response.body)).length; i++) {
        ...
        service.locations =
            ((jsonDecode(response.body))[i])["locations"].cast<String>();
        service.service = new Service();
        service.service.sId =
            ((jsonDecode(response.body))[i])["service"]["_id"];
        service.service.subject = new Subject();
        service.service.subject.sId =
            ((jsonDecode(response.body))[i])["service"]["subject"]["_id"];
        service.service.subject.subject =
            ((jsonDecode(response.body))[i])["service"]["subject"]["subject"];
        service.service.level = new Level();
        service.service.level.sId =
            ((jsonDecode(response.body))[i])["service"]["level"]["_id"];
        service.service.level.level =
            ((jsonDecode(response.body))[i])["service"]["level"]["level"];

        service.service.systemLanguage = new SystemLanguage();
        service.service.systemLanguage.sId =
            ((jsonDecode(response.body))[i])["service"]["systemLanguage"]
                ["_id"];
        service.service.systemLanguage.language =
            ((jsonDecode(response.body))[i])["service"]["systemLanguage"]
                ["language"];
  ...
  Future<bool> GetRequests() async {
    ...
    if (response.statusCode == 200) {
      for (int i = 0; i < (jsonDecode(response.body)).length; i++) {
        ...
        request.student.sId =
            ((jsonDecode(response.body))[i])["student"]["_id"];
        request.student.email =
            ((jsonDecode(response.body))[i])["student"]["email"];
        request.student.name =
            ((jsonDecode(response.body))[i])["student"]["name"];
        request.student.phoneNumber =
            ((jsonDecode(response.body))[i])["student"]["phoneNumber"];

        request.service = new Service();
        request.service.sId =
            ((jsonDecode(response.body))[i])["service"]["_id"];
        request.service.subject = new Subject();
        request.service.subject.sId =
            ((jsonDecode(response.body))[i])["service"]["subject"]["_id"];
        request.service.subject.subject =
            ((jsonDecode(response.body))[i])["service"]["subject"]["subject"];
        request.service.level = new Level();
        request.service.level.sId =
            ((jsonDecode(response.body))[i])["service"]["level"]["_id"];
        request.service.level.level =
            ((jsonDecode(response.body))[i])["service"]["level"]["level"];
        request.service.systemLanguage = new SystemLanguage();
        request.service.systemLanguage.sId =
            ((jsonDecode(response.body))[i])["service"]["systemLanguage"]
                ["_id"];
        request.service.systemLanguage.language =
            ((jsonDecode(response.body))[i])["service"]["systemLanguage"]
                ["language"];

        request.sessionType = ((jsonDecode(response.body))[i])["sessionType"];
        request.sessionDuration =
            ((jsonDecode(response.body))[i])["sessionDuration"];
  ...
  Future<bool> GetTeacherInfo() async {
    ...
    if (response.statusCode == 200) {
      ...
      for (int j = 0;
          j < (jsonDecode(response.body))["availability"].length;
          j++) {
